...
from iot_backend.crud import create_metrics_bulk, get_user_accessible_sources, get_user_by_username
            ...
            if "temperature" in metrics_dict or "humidity" in metrics_dict:
                try:
                    ...
                    try:
                        ...
                        metric_ts = _parse_metric_timestamp(metrics_dict.get("timestamp"))
                        if metrics_dict.get("temperature") is not None:
                            check_and_trigger_metric_alert(
                                db,
                                metric_type="temperature",
                                source=normalized_source,
                                value=float(metrics_dict.get("temperature")),
                                metric_ts=metric_ts,
                                origin="sensor_reading websocket",
                            )
                        if metrics_dict.get("humidity") is not None:
                            check_and_trigger_metric_alert(
                                db,
                                metric_type="humidity",
                                source=normalized_source,
                                value=float(metrics_dict.get("humidity")),
                                metric_ts=metric_ts,
                                origin="sensor_reading websocket",
                            )
                        db.commit()
