...
Flow:
  ...
  - It uses the same forecast/refit logic as 10_generate_7day_forecast.py, scoped to humidity.
...
def print_forecast_plan(devices, metrics_pdf):
    ...
    print("Forecast targets: " + ", ".join(FORECAST_TARGETS))

    locations = sorted(devices["location_id"].astype(str).dropna().unique().tolist())
    for location_id in locations:
        for target_variable in FORECAST_TARGETS:
...
def replacement_predicate(rows_pdf):
    metrics = sorted(rows_pdf["metric_type"].dropna().astype(str).unique().tolist())
    locations = sorted(rows_pdf["location_id"].dropna().astype(str).unique().tolist())
    predicates = [
        "metric_type IN (" + ", ".join(sql_string_literal(item) for item in metrics) + ")",
    ]
    if locations:
        predicates.append("location_id IN (" + ", ".join(sql_string_literal(item) for item in locations) + ")")
    return " AND ".join(predicates)


def write_metric_rows(df, table_name, rows_pdf):
    full_table_name = fq_table(table_name)
    if table_exists(table_name):
        predicate = replacement_predicate(rows_pdf)
        spark.sql("DELETE FROM " + full_table_name + " WHERE " + predicate)  # type: ignore[name-defined]
        df.write.format("delta").mode("append").saveAsTable(full_table_name)
        return

    df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").partitionBy("metric_type").saveAsTable(full_table_name)
...
def main():
    ...
    for _, device in devices.iterrows():
        ...
        for metric_type in FORECAST_TARGETS:
            metric_row = select_best_model(metrics_pdf, metric_type, location_id)
            model = None
            if (
                metric_row
                and metric_row.get("model_type") == "spark_mllib"
                and bool_setting("FORECAST_FORCE_LOCAL_REFIT", "forecast_force_local_refit")
            ):
    ...
    forecast_df = spark.createDataFrame(forecast_pdf)  # type: ignore[name-defined]
    write_metric_rows(forecast_df, "gold_forecast_result", forecast_pdf)

    calibration_pdf = pd.DataFrame(calibration_rows)
    calibration_df = spark.createDataFrame(calibration_pdf)  # type: ignore[name-defined]
    write_metric_rows(calibration_df, "device_calibration_profile", calibration_pdf)
