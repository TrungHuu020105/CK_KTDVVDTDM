...
class MetricCreate(BaseModel):
    ...
    location: Optional[str] = Field(
        default=None,
        max_length=255,
        description="User-facing sensor label/location (for example: 'Living Room' or 'Goc duoi vuon')",
    )
...
class AlertCreate(BaseModel):
    ...
    status: str = Field(..., description="Alert status: 'warning', 'critical', or 'OUT_OF_RANGE'")
    ...
    def validate_status(cls, v):
        """Validate status"""
        if v not in {"warning", "critical", "OUT_OF_RANGE"}:
